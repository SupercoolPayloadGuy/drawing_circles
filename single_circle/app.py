"""
app.py — animation state machine and main loop for single_circle.
"""

import math
import os
import time

import pygame

from config   import WIDTH, HEIGHT, FPS, LINE_WIDTH, BG
from config   import CIRCLE_DRAW_SPEED, POINT_APPEAR_DELAY, Config
from config   import DRAW_MAIN_CIRCLE, SHOW_POINTS, DRAW_FLOWER, DONE
from geometry import circle_from_points
from renderer import create_mask, render_frame, export_png, export_svg
from renderer import draw_button, draw_hint
from intro    import run_intro


BTN_W = 130
BTN_H = 36
BTN_GAP = 8


def _make_top_buttons(ww):
    btns = []
    x = 16
    for act, lbl in [("menu", "\u2190 MENU"), ("jpg", "EXPORT JPG"), ("svg", "EXPORT SVG")]:
        btns.append({"rect": pygame.Rect(x, 16, BTN_W, BTN_H),
                      "action": act, "label": lbl})
        x += BTN_W + BTN_GAP
    return btns


BTN_W_BIG = 130
BTN_H_BIG = 44
BTN_GAP_BIG = 16


def _make_bottom_buttons(ww, wh):
    actions = ["menu", "look"]
    labels = ["MENU", "LOOK"]
    total_w = len(actions) * BTN_W_BIG + (len(actions) - 1) * BTN_GAP_BIG
    sx = (ww - total_w) // 2
    btns = []
    for i, (act, lbl) in enumerate(zip(actions, labels)):
        x = sx + i * (BTN_W_BIG + BTN_GAP_BIG)
        btns.append({"rect": pygame.Rect(x, wh - 80, BTN_W_BIG, BTN_H_BIG),
                      "action": act, "label": lbl})
    return btns


def _export_jpg(screen):
    ts = time.strftime("%Y%m%d_%H%M%S")
    base = os.path.join(os.getcwd(), "map", "exports")
    os.makedirs(base, exist_ok=True)
    fn = os.path.join(base, f"circle_flower_{ts}.jpg")
    export_png(screen, fn)
    return fn


def _export_svg(center, points, circles, circle_count, mr, scheme, ww, wh):
    ts = time.strftime("%Y%m%d_%H%M%S")
    base = os.path.join(os.getcwd(), "map", "exports")
    os.makedirs(base, exist_ok=True)
    fn = os.path.join(base, f"circle_flower_{ts}.svg")
    export_svg(fn, center, points, circles, list(range(circle_count)),
               mr, circle_count, LINE_WIDTH, scheme, ww, wh)
    return fn


def run_animation(screen, clock, cfg: Config):
    ww, wh = screen.get_size()
    center = (ww // 2, wh // 2)
    mr = int(min(ww, wh) * cfg.main_radius / 1200)

    n = cfg.point_count
    points = [
        (
            center[0] + mr * math.cos(-math.pi / 2 + i * (2 * math.pi / n)),
            center[1] + mr * math.sin(-math.pi / 2 + i * (2 * math.pi / n)),
        )
        for i in range(n)
    ]

    circles = [
        {"center": c, "radius": r}
        for p in points
        for c, r in [circle_from_points(center, p)]
    ]

    mask = create_mask(center, ww, wh, mr)
    drawing = pygame.Surface((ww, wh), pygame.SRCALPHA)

    stage = DRAW_MAIN_CIRCLE
    main_progress = 0
    visible_points = 0
    point_timer = 0
    current_circle = 0
    current_progress = 0
    finished_circles = []

    camera_active = False
    captured = None
    cam_off_x = 0
    cam_off_y = 0
    cam_zoom = 1.0
    dragging = False
    drag_start = None

    top_buttons = _make_top_buttons(ww)
    bottom_buttons = _make_bottom_buttons(ww, wh)
    clean_frame = None

    running = True
    while running:
        dt = clock.tick(FPS) / 1000

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False

            elif event.type == pygame.VIDEORESIZE:
                ww, wh = event.w, event.h
                screen = pygame.display.set_mode((ww, wh), pygame.RESIZABLE)
                center = (ww // 2, wh // 2)
                mr = int(min(ww, wh) * cfg.main_radius / 1200)
                points = [
                    (
                        center[0] + mr * math.cos(-math.pi / 2 + i * (2 * math.pi / n)),
                        center[1] + mr * math.sin(-math.pi / 2 + i * (2 * math.pi / n)),
                    )
                    for i in range(n)
                ]
                circles = [
                    {"center": c, "radius": r}
                    for p in points
                    for c, r in [circle_from_points(center, p)]
                ]
                mask = create_mask(center, ww, wh, mr)
                drawing = pygame.Surface((ww, wh), pygame.SRCALPHA)
                stage = DRAW_MAIN_CIRCLE
                main_progress = 0
                visible_points = 0
                point_timer = 0
                current_circle = 0
                current_progress = 0
                finished_circles = []
                clean_frame = None
                camera_active = False
                top_buttons = _make_top_buttons(ww)
                bottom_buttons = _make_bottom_buttons(ww, wh)

            elif event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_ESCAPE, pygame.K_m):
                    if camera_active:
                        camera_active = False
                        cam_zoom = 1.0
                        cam_off_x = cam_off_y = 0
                    else:
                        return
                elif event.key == pygame.K_p:
                    print(f"exported {_export_jpg(screen)}")
                elif event.key == pygame.K_s:
                    print(f"exported {_export_svg(center, points, circles, current_circle, mr, cfg.color_scheme, ww, wh)}")

            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                for btn in top_buttons:
                    if btn["rect"].collidepoint(event.pos):
                        act = btn["action"]
                        if act == "menu":
                            return
                        elif act == "jpg":
                            print(f"exported {_export_jpg(screen)}")
                        elif act == "svg":
                            print(f"exported {_export_svg(center, points, circles, current_circle, mr, cfg.color_scheme, ww, wh)}")

            if stage == DONE or camera_active:
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    pos = event.pos
                    if camera_active:
                        dragging = True
                        drag_start = pos
                    else:
                        for btn in bottom_buttons:
                            if btn["rect"].collidepoint(pos):
                                act = btn["action"]
                                if act == "menu":
                                    return
                                elif act == "look":
                                    if clean_frame is not None:
                                        captured = clean_frame
                                        camera_active = True
                                        cam_zoom = 1.0
                                        cam_off_x = cam_off_y = 0

                elif event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    dragging = False

                elif event.type == pygame.MOUSEMOTION and dragging and camera_active:
                    dx = event.pos[0] - drag_start[0]
                    dy = event.pos[1] - drag_start[1]
                    cam_off_x += dx
                    cam_off_y += dy
                    drag_start = event.pos

                elif event.type == pygame.MOUSEWHEEL and camera_active:
                    mx, my = pygame.mouse.get_pos()
                    zf = 1.1 if event.y > 0 else 0.9
                    sw = int(ww * cam_zoom)
                    sh = int(wh * cam_zoom)
                    img_x = mx - (ww // 2 - sw // 2 + int(cam_off_x))
                    img_y = my - (wh // 2 - sh // 2 + int(cam_off_y))
                    frac_x = img_x / sw if sw > 0 else 0.5
                    frac_y = img_y / sh if sh > 0 else 0.5
                    cam_zoom = max(0.1, min(10.0, cam_zoom * zf))
                    nsw = int(ww * cam_zoom)
                    nsh = int(wh * cam_zoom)
                    cam_off_x = mx - (ww // 2 - nsw // 2 + frac_x * nsw)
                    cam_off_y = my - (wh // 2 - nsh // 2 + frac_y * nsh)

        if stage == DRAW_MAIN_CIRCLE:
            main_progress += CIRCLE_DRAW_SPEED * cfg.speed * dt
            if main_progress >= 360:
                main_progress = 360
                stage = SHOW_POINTS

        elif stage == SHOW_POINTS:
            point_timer += dt
            if point_timer >= POINT_APPEAR_DELAY / cfg.speed:
                point_timer = 0
                visible_points += 1
                if visible_points >= len(points):
                    visible_points = len(points)
                    stage = DRAW_FLOWER

        elif stage == DRAW_FLOWER:
            if current_circle < len(circles):
                current_progress += CIRCLE_DRAW_SPEED * cfg.speed * dt
                if current_progress >= 360:
                    finished_circles.append(circles[current_circle])
                    current_circle += 1
                    current_progress = 0
            else:
                stage = DONE

        if camera_active and captured is not None:
            screen.fill(BG)
            cw, ch = captured.get_size()
            sw = int(cw * cam_zoom)
            sh = int(ch * cam_zoom)
            scaled = pygame.transform.smoothscale(captured, (sw, sh))
            screen.blit(scaled, (ww // 2 - sw // 2 + int(cam_off_x),
                                 wh // 2 - sh // 2 + int(cam_off_y)))
            for btn in bottom_buttons:
                draw_button(screen, btn["rect"], btn["label"])
            draw_hint(screen, "drag to pan  \u00b7  scroll to zoom  \u00b7  ESC to exit", ww)
        else:
            render_frame(screen, drawing, mask, center,
                         main_progress, finished_circles,
                         current_circle, circles,
                         current_progress, stage,
                         points, visible_points,
                         cfg.color_scheme, cfg.show_circle_count,
                         cfg.show_progress_bar, ww, wh, mr)

            for btn in top_buttons:
                draw_button(screen, btn["rect"], btn["label"])

            if stage == DONE and not camera_active:
                clean_frame = screen.copy()
                for btn in bottom_buttons:
                    draw_button(screen, btn["rect"], btn["label"])

        pygame.display.flip()

    pygame.quit()
    raise SystemExit


def run():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
    clock = pygame.time.Clock()

    cfg = Config()
    while True:
        cfg = run_intro(screen, clock, cfg)
        run_animation(screen, clock, cfg)
